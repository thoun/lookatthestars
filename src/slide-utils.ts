function slideToObjectAndAttach(game: LookAtTheStarsGame, object: HTMLElement, destinationId: string, zoom: number = 1): Promise<boolean> {
...
function slideToObjectTicketSlot2(game: LookAtTheStarsGame, object: HTMLElement, destinationId: string, keepTransform: string): Promise<boolean> {
    const destination = document.getElementById(destinationId);
    if (destination.contains(object)) {
        return Promise.resolve(true);
    }
